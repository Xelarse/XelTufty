...
namespace pimoroni {
    PicoGraphics_PenRGB556::PicoGraphics_PenRGB556(uint16_t width, uint16_t height, void *frame_buffer, uint16_t layers)
    ...
    void PicoGraphics_PenRGB556::set_pen(uint c) {
        color = c;
    }
    void PicoGraphics_PenRGB556::set_pen(uint8_t r, uint8_t g, uint8_t b) {
        color = rgb_to_rgb332(r, g, b);
    }
    int PicoGraphics_PenRGB556::create_pen(uint8_t r, uint8_t g, uint8_t b) {
        return rgb_to_rgb332(r, g, b);
    }
    int PicoGraphics_PenRGB556::create_pen_hsv(float h, float s, float v) {
        return RGB::from_hsv(h, s, v).to_rgb332();
    }
    void PicoGraphics_PenRGB556::set_pixel(const Point &p) {
        uint8_t *buf = (uint8_t *)frame_buffer;
        buf += this->layer_offset;
        buf[p.y * bounds.w + p.x] = color;
    }
    void PicoGraphics_PenRGB556::set_pixel_span(const Point &p, uint l) {
    ...
    void PicoGraphics_PenRGB556::set_pixel_alpha(const Point &p, const uint8_t a) {
    ...
    void PicoGraphics_PenRGB556::set_pixel_dither(const Point &p, const RGB &c) {
        if(!bounds.contains(p)) return;
    ...
    void PicoGraphics_PenRGB556::set_pixel_dither(const Point &p, const RGB565 &c) {
        if(!bounds.contains(p)) return;
    ...
    void PicoGraphics_PenRGB556::frame_convert(PenType type, conversion_callback_func callback) {
        if(type == PEN_RGB565) {
    ...
    void PicoGraphics_PenRGB556::sprite(void* data, const Point &sprite, const Point &dest, const int scale, const int transparent) {
    ...
    bool PicoGraphics_PenRGB556::render_tile(const Tile *tile) {
